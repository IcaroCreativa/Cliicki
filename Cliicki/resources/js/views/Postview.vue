...
export default {
    ...
    methods: {
        ...
        storeStars(post_id,stars){
            ...
            .then(result => {console.log(result); if (result[0] ==1) {this.$swal("Vous avez déjà noté ce post").then(result=>{location.reload();});}
            else{
                this.$swal("Vous avez noté ce post").then(result=>{location.reload();})
